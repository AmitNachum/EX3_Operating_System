...
int main() {
    ...
    int opt = 1;
    int sock_check = setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    if(sock_check == -1){
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
    ...
    while (true) {
                ...
                Proactor pro(client_fd,handle_client);
                ++thread_count;
